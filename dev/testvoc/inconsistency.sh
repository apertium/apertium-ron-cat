...
lt-expand $2 | grep -v REGEX | grep -v ':<:' | sed 's/:>:/\'$'\t/g' | sed 's/:/\'$'\t/g' | cut -f2 -d$'\t' | sed 's/^/^/g' | sed 's/$/$/g' | tee $TMPDIR/tmp_testvoc1-$1.txt |\
        ...
        apertium-transfer -b ../../apertium-ron-cat.cat-ron.t1x  ../../cat-ron.t1x.bin | apertium-interchunk ../../apertium-ron-cat.cat-ron.t2x  ../../cat-ron.t2x.bin | apertium-postchunk ../../apertium-ron-cat.cat-ron.t3x  ../../cat-ron.t3x.bin | tee $TMPDIR/tmp_testvoc2-$1.txt |\
